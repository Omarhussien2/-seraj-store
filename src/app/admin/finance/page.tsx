...
export default function AdminFinancePage() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [summaryRes, productsRes, expensesRes, reportsRes, settingsRes] = await Promise.all([
      ...
      const [summaryJson, productsJson, expensesJson, reportsJson, settingsJson] =
        await Promise.all([
          summaryRes.json(),
          productsRes.json(),
          expensesRes.json(),
        ...
        <TabsContent value="expenses" className="space-y-4">
          <Card>
            ...
            <CardContent className="grid grid-cols-1 gap-3 md:grid-cols-6">
              ...
              />
              <Input
                placeholder="Slug المنتج"
                value={expenseForm.productSlug}
                disabled={expenseForm.scope !== "product"}
                onChange={(e) =>
                  setExpenseForm((prev) => ({ ...prev, productSlug: e.target.value }))
                }
              />
              <Button
                type="button"
