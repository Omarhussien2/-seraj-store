...
| الخيار | النص المعتمد |
|--------|-------------|
| **عربون** | "ادفعي 50 جنيه بس وسراج هيبدأ شغل، والباقي بعد ما تشوفي البروفة" |
...
- **الخطوات:**
  1. نموذج بيانات الشحن (React Hook Form + Zod):
     ...
     - العنوان (مطلوب)
     - ملاحظات (اختياري)
  2. ملخص الطلب: المنتجات + الإجمالي
  3. خيار الدفع:
     - عربون 50 جنيه: "ادفعي 50 جنيه بس والباقي بعد البروفة"
     - VIP كامل: "خلصي الدفع دلوقتي وأولوية التنفيذ"
  4. عرض بيانات InstaPay: الرقم + QR Code
  5. عند التأكيد: `POST /api/orders` → يحفظ الطلب → redirect لصفحة النجاح
  6. لو السلة فاضية → redirect للرئيسية
